...
def get_gradzero(model, valdata, vallabel, loss_function, optimizer, revert=False):
    # validation_data, validation_label = self.data_reader.get_batch(self.data_reader.fl_trust.type(torch.int64))
    cache = get_flatten_parameters(model).to(device)
    batch_size = 250
    # for i in range(len(valdata)):
    #     i = i-1
    #     valdata = valdata[
    #                          i  * batch_size: 1* batch_size].to(device)
    #     vallabel = vallabel[
    #                      i  * batch_size: 1* batch_size].to(device)
    #     out = model(valdata).to(device)
    #     loss = loss_function(out, vallabel)
    #     optimizer.zero_grad()
    #     loss.backward()
        # model_grads = []
        # start_idx = 0
    ...
    for param in model.parameters():
        param_grad = param.grad.data.view(-1) if not len(param_grad) else torch.cat((param_grad, param.grad.view(-1)))
    optimizer.step(param_grad)
    gradient = get_flatten_parameters(model) - cache
    if revert:
