...
from models.cifar.mnist_cnn import SimpleCNNMNIST
from models.cifar.cnn import SubnetConv,SubnetLinear, SimpleCNN
from models.cifar.alexnet import AlexNet_hydra
...
def get_model(config, parallel=False, cuda=True, device=0):
    ...
    elif config['arch'].endswith('alexnet_hydra'):
        cl = get_layers()
        model = AlexNet_hydra(cl, num_classes=config['num_classes'])
    elif config['arch'].endswith('simplecnn'):
        cl, ll = get_layers()
        model = SimpleCNN(cl, ll, input_dim=config['input_dim'], hidden_dims=config['hidden_dims'])
    elif config['arch'].endswith('simplecnnmnist'):
        cl, ll = get_layers()
        model = SimpleCNNMNIST(cl, ll, input_dim=config['input_dim'], hidden_dims=config['hidden_dims'])
    else:
...
def return_model(model_name, lr, momentum, parallel=False, cuda=True, device=0):
    if model_name == 'dc':
        ...
        model = get_model(arch_config, parallel=parallel, cuda=cuda, device=device)
        optimizer = optim.SGD(model.parameters(), lr=0.001, momentum=momentum)
    elif model_name == 'simplecnnmnist':
        arch_config = {
            'arch': 'simplecnnmnist',
            'input_dim': (16 * 4 * 4),
            'hidden_dims': [120, 84],
