...
def get_semisup_dataloader(semisup_data, semisup_fraction, transform):
    if semisup_data == "splitgan":
        print(f"Loading {semisup_data} generated data")
        img, label = (
            np.load(
    ...
    if semisup_data == "tinyimages":
        data_dir = "./datasets"
        print(f"Loading {semisup_data} dataset")
        with open(
            os.path.join(data_dir, "tiny_images/ti_top_50000_pred_v3.1.pickle"),
            "rb",
        ) as f:
            data = pickle.load(f)
    ...
    index = np.random.permutation(np.arange(len(label)))[
        0 : int(semisup_fraction * len(label))
    ]
